...
from apiTestIHRMSystem.uitls import assert_common_utils, DBUtils, read_add_emp, read_query_emp, read_modify_emp, \
    read_delete_emp
...
class TestEmployee(unittest.TestCase):
    ...
    def test02_add_emp(self, username, mobile, http_code, success, code, message):
        logging.info("保存在类方法的属性值{}".format(apiTestIHRMSystem.app.HEADERS))
        # 调用添加员工
    ...
    def test04_modify_emp(self,username,http_code, success, code, message):
        # 调用修改员工
        response_modify = self.emp_api.modify_emp(apiTestIHRMSystem.app.EMPID, username, headers=apiTestIHRMSystem.app.HEADERS)
